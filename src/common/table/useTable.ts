...
async function queryEntityOverview(
  setRows: React.SetStateAction<any>,
  listRowObject: Promise<any>,
  objectShortQueryName: string,
  entityOverviewQueryProcessingFunc?: (data: IValues[]) => IValues[]
) {
  try {
    await listRowObject.then((rowObjects: any) => {
      console.log("rowObjects -> ", rowObjects);
      console.log("objectShortQueryName -> ", objectShortQueryName);
