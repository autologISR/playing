...
import { newRateSchema } from "./Alpha/NewRateFormAlpha";
...
export const ratesOverviewQueryProcessingFunc = (data: IValues[]) => {
  return data.map((rate) => {
    ...
    const modeOfTransport =
      incoterm === "COURIER" ? "N/A" : rateTypeSubtypes[3];
...
const ratesOverview = API.graphql(
  graphqlOperation(queries.listRateSubmissions, Predicates.ALL)
);
...
const domainSchemaAlpha: DomainSchema = new Map([
  [
    "Current Rates",
    {
      entityName: "Current Rates",
...
export const RatesDashboardSection: React.FunctionComponent<RatesTableProps[]> = () => {
  return (
    <AutologDashboardSection
      entityNames={["Current Rates"]}
      domainSchema={domainSchemaAlpha}
    />
  );
};
